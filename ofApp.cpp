	...
	for (int z = -64; z <= 64; z += 16) {
		...
		for (int radius = 180; radius <= 360; radius += 30) {
			...
			for (int deg = 0; deg < 360 + tmp_deg; deg += 1) {
				...
				auto noise_value = ofNoise(glm::vec4(radius * cos(deg * DEG_TO_RAD) * 0.005, radius * sin(deg * DEG_TO_RAD) * 0.005, z * 0.05, ofGetFrameNum() * 0.01));
				...
				if (noise_value < 0.45) {
					...
					if (deg == 0) {
						...
						while (true) {
							...
							tmp_deg -= 1;
							auto tmp_noise_value = ofNoise(glm::vec4(radius * cos(tmp_deg * DEG_TO_RAD) * 0.005, radius * sin(tmp_deg * DEG_TO_RAD) * 0.005, z * 0.05, ofGetFrameNum() * 0.01));
					...
					if (deg_start < deg_end) {
						...
						this->setRingToMesh(this->face, this->frame, glm::vec3(0, 0, z + 8), radius, 30, 16, deg_start, deg_end);
					}
...
void ofApp::draw() {
	...
	this->cam.begin();
	ofRotateX(ofGetFrameNum() * 0.62);
	ofRotateY(ofGetFrameNum() * 1.4);
